...
<div class="ft__on-surface ft__smaller">
    ...
    ${
        isIOS
            ? `<a href="javascript:void(0)" data-action="iOSPay" data-type="subscribe">${window.siyuan.languages.clickMeToRenew}</a>`
...
${renewHTML}<div class="fn__hr--b"></div>`
                ...
                if (window.siyuan.user.userSiYuanOneTimePayStatus === 0) {
                    subscriptionHTML += isIOS
                        ? `<button class="b3-button b3-button--success" data-action="iOSPay" data-type="function">
    <svg><use xlink:href="#iconVIP"></use></svg>${window.siyuan.languages.onepay}
</button>`
                        : `<a class="b3-button b3-button--success" href="${getIndexURL('pricing.html')}" target="_blank">
    <svg><use xlink:href="#iconVIP"></use></svg>${window.siyuan.languages.onepay}
</a>`
                }
            } else {
...
<div class="b3-form__space config-account__center">
    <div class="config-account__form" id="form1">
        ...
        <div class="ft__center">
            ...
            <a href="${getCloudURL('register')}" class="b3-button b3-button--cancel${window.siyuan.config.system.container === 'ios' ? ' fn__none' : ''}" target="_blank">${
...
<div class="config-account__center config-account__center--text${window.siyuan.config.system.container === 'ios' ? ' fn__none' : ''}">
    ...
    bindEvent: (element: Element) => {
        ...
        cloudRegionElement.addEventListener('change', () => {
            ...
            element.querySelector('#form1').lastElementChild.innerHTML = `<a href="${getCloudURL('forget-pwd')}" class="b3-button b3-button--cancel" target="_blank">${
                ...
                (data) => {
                    ...
                    if (data.code === 1) {
                        ...
                        if (data.data.needCaptcha) {
                            ...
                            captchaElement.previousElementSibling.setAttribute('src', getCloudURL('captcha') + `/login?needCaptcha=${data.data.needCaptcha}`)
